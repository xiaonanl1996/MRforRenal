...
TEU_BlP_DBP_quintiles <- function() {
  list(
    name = "TEU_BlP_DBP_quintiles", 
    source = c("TEU_BlP_DBP.avg"), 
    mapper = FN_quantiles(quant=5),
    post_exclusion = TRUE,
    display_name = "Baseline Diastolic BP quintiles",
    description = "Quintiles of the measured diastolic blood pressure at baseline"
  )
}
...
TEU_SBP_PRS <- function() {
  list(
    name = "TEU_SBP_PRS", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/sbp_prs_20211111.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
...
TEU_SBP_PRS_Sens8 <- function() {
  list(
    name = "TEU_SBP_PRS_Sens8", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/sbp_prs_NoBMISmok8_20211129.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
    display_name = "SBP polygenic risk score (Sens)",
    description = "SBP polygenic risk score from Evangelou 2018 paper; with BMI, obesity and smoking assocaited variants removed using pvalue threshold 5e-8 (PhenoScanner)"
  )
}

TEU_SBP_PRS_Sens6 <- function() {
  list(
    name = "TEU_SBP_PRS_Sens6", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/sbp_prs_NoBMISmok6_20211129.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
    display_name = "SBP polygenic risk score (Sens)",
    description = "SBP polygenic risk score from Evangelou 2018 paper; with BMI, obesity and smoking assocaited variants removed using pvalue threshold 5e-6 (PhenoScanner)"
  )
}
...
TEU_DBP_PRS <- function() {
  list(
    name = "TEU_DBP_PRS", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/dbp_prs_20211111.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
...
TEU_DBP_PRS_Sens8 <- function() {
  list(
    name = "TEU_DBP_PRS_Sens8", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/dbp_prs_NoBMISmok8_20211129.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
    display_name = "DBP polygenic risk score (Sens)",
    description = "DBP polygenic risk score from Evangelou 2018 paper; with BMI, obesity and smoking assocaited variants removed using pvalue threshold 5e-8 (PhenoScanner)"
  )
}

TEU_DBP_PRS_Sens6 <- function() {
  list(
    name = "TEU_DBP_PRS_Sens6", 
    source = c("ID"), 
    mapper = FN_JoinPRS(filepath="K:/TEU/UKB_Genetic_Data/PRS_Pipeline/prs/projects/htn-evangelou2018/outputs/dbp_prs_NoBMISmok6_20211129.sscore",
                        colname="SCORE1_SUM"),
    post_exclusion = FALSE,
    display_name = "DBP polygenic risk score (Sens)",
    description = "DBP polygenic risk score from Evangelou 2018 paper; with BMI, obesity and smoking assocaited variants removed using pvalue threshold 5e-6(PhenoScanner)"
  )
}
...
TEU_DBP_PRS_quintiles <- function() {
  list(
    name = "TEU_DBP_PRS_quintiles", 
    source = c("TEU_DBP_PRS"), 
    mapper = FN_quantiles(quant=5),
    post_exclusion = TRUE,
    display_name = "Diastolic BP PRS quintiles",
    description = "Quintiles of the DBP PRS score"
  )
}
  ...
  list(
    ...
    list(
      ...
      mapper = FN_VI_filtercodes(dx_codes = dx_codes,
                                 ...
                                 mapper = file.path(config$cleaning$coding,"coding6_flat_NonCancerIllness.csv")),
      post_exclusion = FALSE,
      display_name = "Self-reported duration of HTN (VI)",
...
TEU_QRISK3 <- function() {
  list(
    ...
    source = "ID",
    mapper = function(x) {
      QRISK3 <- readRDS("K:\\TEU\\MScRP_2021\\Git_Repo\\Data\\Derived\\QRISK3_20210629.rds")
      y <- QRISK3[["QRISK3_2017"]][match(x, QRISK3$ID)]
    },
    post_exclusion = FALSE,
    display_name = "QRISK3 scores",
    description = "QRISK3 scores (complete case) derived based on Elliot2020 mapping, except 'On BP trt' variable where we used TEU definition. 
    Note: Scores are only available for people with no prevalent MACE at baseline."
  )
}


OtherCancerDx_Prevalent<-function(code_list){
  list(
    ...
    mapper = function(data) {
      ...
      ICD10 <- FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                         code_list=code_list, exclude=TRUE,type="prevalent", keepcol="DiagICD10")(data)
      ...
      ICD9 <- FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                        code_list=code_list, exclude=TRUE,type="prevalent", keepcol="DiagICD9")(data)
    ...
    description = "Indicator of any prevalent cancer except non-melanoma skin cancer, CIS (0=no;1=yes)"
  )
  
}

RenalCancerDx_Prevalent<- function(){
  
  codes <- data.frame(ICD10=c("C64","C65","C66","C67"), 
                      ICD9=c("1890","1891","1892","188"),
                      fullnames=c("Renal parenchyma", "Renal pelvis","Ureter","Bladder"))
  conditions <- split(codes, seq(nrow(codes)))
  
  names(conditions)<-c("paren","pelvis","ureter","bladder")
  
  lapply(1:length(conditions), function(i)
    
    list(
      name = paste0(names(conditions[i]),"CancerDx_Prevalent"),
      source =c("ID", "Rec_DateAssess",
                paste0("CaR_DiagDate.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD10.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD9.", seq(0, 14, by=1), ".0")),
      mapper = function(data) {
        codingICD10 <- read.csv(file.path(config$cleaning$coding, "coding19_flat_Icd10.csv"))
        ICD10 <- FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                           code_list=list(ICD10=conditions[[i]]$ICD10, ICD9=conditions[[i]]$ICD9), 
                           exclude=FALSE,type="prevalent", keepcol="DiagICD10")(data)
        meaningICD10 <- codingICD10$meaning[match(ICD10, codingICD10$Code)]
        
        codingICD9 <- read.csv(file.path(config$cleaning$coding, "coding87_flat_Icd9.csv"))
        ICD9 <- FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                          code_list=list(ICD10=conditions[[i]]$ICD10, ICD9=conditions[[i]]$ICD9), 
                          exclude=FALSE,type="prevalent", keepcol="DiagICD9")(data)
        meaningICD9 <- codingICD9$meaning[match(ICD9, codingICD9$Code)]
      ...
      post_exclusion = FALSE,
      display_name = paste0("Prevalent ", names(conditions[i]), " dx"),
      description = paste0("Indicator of ", conditions[[i]]$fullnames, "prevalent cancer")
    )
...
# Renal outcome from HES
TEU_Renal_HES<-function(record_level=TRUE){
  
  conditions<-list("Renal parenchyma", "Renal pelvis","Ureter","Bladder")
  names(conditions)<-c("paren","pelvis","ureter","bladder")
  
    lapply(1:length(conditions), function(i)
      
      list(
        name=paste0('TEU_HES_',names(conditions[i]),'_incdate'),
        source=if(record_level){
        ...
        mapper=function(data){
          ...
          y <- FN_HES_First(ICD9_xlsx = file.path(config$cleaning$mapping,'HES_ICD9_Mapping_20211027.xlsx'),
                       ICD10_xlsx = file.path(config$cleaning$mapping,'HES_ICD10_Mapping_20211027.xlsx'),
                       #OPCS4_xlsx = file.path(config$cleaning$mapping,'MACE/HES_OPCS4_Mapping_20210128.xlsx'),
        ...
        post_exclusion=FALSE,
        display_name=paste0('Incident ',conditions[[i]],' cancer FU date from HES'),
        description=paste0(conditions[[i]],' date identified from HES (ICD-9,10) data at follow-up')
      )
    )
  
}

# Dth by Renal outcome
TEU_Dth_Renal_dthdate <-function(record_level=TRUE){
  ...
  lapply(1:length(conditions), function(i)
    list(
      name = paste0('TEU_Dth_',names(conditions[i]),'_dthdate'),
      source = if(record_level){c("ID")} else {c('ID',"Dth_ICD10Underlying.0.0", "Dth_ICD10Underlying.1.0","Dth_Date.0.0", "Dth_Date.1.0")},
      mapper = function(data){
        
        y<-FN_Dth_filtercodes(ICD10_codes = conditions[[i]]$ICD10,return_label = 'dth_date', record_level=record_level, exclude=FALSE)(data)
        return(y)
      },
      post_exclusion = FALSE,
      display_name = paste0(conditions[[i]]$fullnames,' death date'),
      description = paste0('Death date caused by ', conditions[[i]]$fullnames ,'from Death Registry data')
    )
  )
}


# Incorporate CaR and HES and Dth for cancer outcome
TEU_RenalCancer <- function() {
  ...
  names(conditions)<-c("paren","pelvis","ureter","bladder")
    ...
    lapply(1:length(conditions), function(i)
      ...
      list(
        name=paste0(names(conditions[i]),'Date_Incident'),
        source=c("ID", "Rec_DateAssess", paste0('TEU_HES_',names(conditions[i]),'_incdate'),
                 paste0('TEU_Dth_',names(conditions[i]),'_dthdate'),
                 paste0("CaR_DiagDate.", seq(0, 16, by=1), ".0"),
                 paste0("CaR_DiagICD10.", seq(0, 16, by=1), ".0"),
                 paste0("CaR_DiagICD9.", seq(0, 14, by=1), ".0")),
        mapper=function(data){
          CaR <- FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                           code_list=list(ICD10=conditions[[i]]$ICD10, ICD9=conditions[[i]]$ICD9), type="incident", keepcol="DiagDate")(data)
          event_date <- coalesce(CaR, data[[paste0('TEU_HES_',names(conditions[i]),'_incdate')]])
          
          y <- pmin(event_date,data[[paste0('TEU_Dth_',names(conditions[i]),'_dthdate')]],na.rm = TRUE)
          return(y)
        },
        post_exclusion=FALSE,
        display_name=paste0('Incident ',conditions[[i]]$fullnames,' dx date'),
        description=paste0("Date of first ",conditions[[i]]$fullnames,' at follow-up from CaR supplemented with HES or death date')
      )
    )
...
# Dth by other causes
TEU_Dth_NotRenal_dthdate <-function(record_level=TRUE){
  ...
  lapply(1:length(conditions), function(i)
  list(
    name = paste0('TEU_Dth_Not',names(conditions[i]),'_dthdate'),
    source = if(record_level){c("ID")} else {c('ID',"Dth_ICD10Underlying.0.0", "Dth_ICD10Underlying.1.0","Dth_Date.0.0", "Dth_Date.1.0")},
    mapper = function(data){
      
      y<-FN_Dth_filtercodes(ICD10_codes = conditions[[i]]$ICD10,return_label = 'dth_date', record_level=record_level, exclude=TRUE)(data)
      return(y)
    },
    post_exclusion = FALSE,
    display_name = paste0('Non ',conditions[[i]]$fullnames,' death date'),
    description = paste0('Death date caused by non', conditions[[i]]$fullnames ,'from Death Registry data')
  )
  )
}
...
Admin_CensorDate_RenalHES <- function(){
  list(
    name = 'Admin_CensorDate',
...
Renal_censordate<-function(){
  
  conditions<-c("paren","pelvis","ureter","bladder")
  
  lapply(1:length(conditions), function(i)
  list(
    name = paste0(conditions[i],'_censordate'),
    source = c(paste0('TEU_Dth_Not',conditions[i],'_dthdate'),'Admin_CensorDate','BaC_LostFUDate'),
    mapper = function(data){
      y<-pmin(data[[paste0('TEU_Dth_Not',conditions[i],'_dthdate')]],
              data$Admin_CensorDate,data$BaC_LostFUDate,
              na.rm = TRUE)
      return(y)
    },
    post_exclusion = FALSE,
    display_name = paste0(conditions[i],' censoring date'),
    description = 'Censoring date for renal outcome'
  )
  )
}

# Renal outcomes (0=censored, 1=event)
TEU_Renal_status<-function(){
  ...
  lapply(1:length(conditions), function(i)
    ...
    list(
      name = paste0('TEU_',names(conditions[i]),'_status'),
      source = c(paste0(names(conditions[i]),'_censordate'),
                 paste0(names(conditions[i]),'Date_Incident')),
      mapper = function(data){
        # Check if censoring date has NA
        if (anyNA(data[[paste0(names(conditions[i]),'_censordate')]])==TRUE){
          warning('Missing Censoring Date: Need to double check!')
        }
        data<-data%>%
          # Rename to make var name slightly visable
          rename_at(vars(ends_with("_censordate")),list(~paste0("censordate")))%>%
          rename_at(vars(ends_with("Date_Incident")),list(~paste0("Date_Incident")))%>%
          mutate(status=case_when(
            !is.na(Date_Incident) & Date_Incident<=censordate ~ 1,
            is.na(Date_Incident) |(!is.na(Date_Incident)&Date_Incident>censordate) ~ 0))
        
        return(data$status)
        
      },
      post_exclusion = FALSE,
      display_name = paste0(conditions[[i]],' cancer status'),
      description = paste0('Event status of ',conditions[[i]],'(0=censored, 1=event)')
...
TEU_Renal_time<-function(){
  ...
  lapply(1:length(conditions), function(i)
    ...
    list(
      name = paste0('TEU_',names(conditions[i]),'_time'),
      source = c(paste0(names(conditions[i]),'_censordate'),
                 paste0(names(conditions[i]),'Date_Incident'),
                 'Rec_DateAssess',
                 paste0('TEU_',names(conditions[i]),'_status')),
      mapper = function(data){
        
        data=data %>%
          rename_at(vars(ends_with("_status")),list(~paste0("status")))%>%
          rename_at(vars(ends_with("_censordate")),list(~paste0("censordate")))%>%
          rename_at(vars(ends_with("Date_Incident")),list(~paste0("Date_Incident")))%>%
          mutate(
            time=case_when(
              status==0 ~ as.numeric(difftime(censordate, Rec_DateAssess, unit='days'))/365.25,
              status==1 ~ as.numeric(difftime(Date_Incident, Rec_DateAssess, unit='days'))/365.25
            )
          ) %>%
          mutate(
...
TEU_comp_status<-function(){
  list(
    name = 'TEU_comp_status',
    source = c('TEU_paren_status','TEU_pelvis_status'),
    mapper = function(data){
      
      data<-data%>%
        mutate(status=ifelse(TEU_paren_status==1|TEU_pelvis_status==1,1,0))
      
      return(data$status)
      
    },
    post_exclusion = FALSE,
    display_name = 'Composite renal cancer status',
    description = 'Event status of composite of renal parenchyma and pelvis (0=censored, 1=event)'
...
TEU_comp_time <- function(){
  list(
    name = 'TEU_comp_time',
    source = c("TEU_paren_time", "TEU_pelvis_time"),
    mapper = function(data){
      y <- pmin(data$TEU_paren_time, data$TEU_pelvis_time)
      return(y)
    },
    post_exclusion = FALSE,
    display_name = 'Composite renal cancer follow up time',
    description = 'If event status=0, this fields returns time difference in days between censoring date and baseline date.
    If event status=1, this fields returns time to composite of renal parenchyma and pelvis event.'
  )
}


# For exploration: First dx date (could be either prevalent or incident)
RenalOutcome_Dates<- function(){
  
  list(
    list(
      name = "RenalCancerDx_Date",
      source =c("ID", "Rec_DateAssess",
                paste0("CaR_DiagDate.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD10.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD9.", seq(0, 14, by=1), ".0")),
      mapper = FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                           code_list=list(ICD10=c("C64","C65"), ICD9=c("1890","1891")), 
                           exclude=FALSE, keepcol="DiagDate"),
      post_exclusion = FALSE,
      display_name = "Renal date",
      description = "Renal date"
    ),
    list(
      name = "UreterCancerDx_Date",
      source =c("ID", "Rec_DateAssess",
                paste0("CaR_DiagDate.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD10.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD9.", seq(0, 14, by=1), ".0")),
      mapper = FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                         code_list=list(ICD10=c("C66"), ICD9=c("1892")), 
                         exclude=FALSE, keepcol="DiagDate"),
      post_exclusion = FALSE,
      display_name = "Ureter date",
      description = "Ureter date"
    ),
    list(
      name = "BladderCancerDx_Date",
      source =c("ID", "Rec_DateAssess",
                paste0("CaR_DiagDate.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD10.", seq(0, 16, by=1), ".0"),
                paste0("CaR_DiagICD9.", seq(0, 14, by=1), ".0")),
      mapper = FN_Cancer(colgroup="CaR_", not_na="DiagICD", 
                         code_list=list(ICD10=c("C67"), ICD9=c("188")), 
                         exclude=FALSE, keepcol="DiagDate"),
      post_exclusion = FALSE,
      display_name = "Bladder date",
      description = "Bladder date"
    )
