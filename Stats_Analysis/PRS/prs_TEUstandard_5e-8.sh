...
SBP_PRS=sbp_prs_NoBMISmok8_20211129
DBP_PRS=dbp_prs_NoBMISmok8_20211129
...
# Combine LD and variants associated with confounders into one txt (can't have duplicate --exclude)
cat ./outputs/AfterQC.prune.out ./outputs/BmiObsSmok_5e-8.txt > ./outputs/ld_bmi_obs_smok.txt

plink2 --pfile outputs/raw \
--memory 15000 \
--extract outputs/snpQC.snplist \
--exclude outputs/ld_bmi_obs_smok.txt \
--keep-fam ${USEDINPCA} \
